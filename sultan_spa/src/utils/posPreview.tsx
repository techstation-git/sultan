...
import { dbGet, APP_CACHE_STORE } from "../services/offlineDB";
import { compileOfflinePrintFormat } from "./printCompiler";
...
function generateOfflineReceiptHTML(invoice: any, posDetails: any, language: "en" | "ar" = "en", qrDataUrl?: string, barcodeSvgHtml?: string) {
  ...
  const itemsHtml = items.map((item: any) => {
    ...
    if (item.item_tax_rate) {
      try {
        const ratesObj = typeof item.item_tax_rate === 'string' ? JSON.parse(item.item_tax_rate) : item.item_tax_rate;
        const rates = Object.values(ratesObj);
        if (rates.length > 0) {
          taxRate = Number(rates[0]) / 100;
        }
      } catch (e) {
        console.warn("Failed to parse item_tax_rate:", e);
      }
    }

    let rate = item.rate ?? item.price ?? 0;
    if (posDetails?.custom_prices_include_vat) {
      if (taxRate > 0 && item.rate) {
        rate = item.rate * (1 + taxRate);
      } else if (item.price) {
        rate = item.price;
      }
    }
    ...
    return `
      <tr style="border-bottom: 1px dashed #eee;">
        ...
        <td class="c bold" style="vertical-align: middle;">${Number(qty).toFixed(2)}</td>
        <td class="r bold" style="vertical-align: middle;">${Number(rate).toFixed(2)}</td>
        <td class="r bold" style="vertical-align: middle;">${Number(amount).toFixed(2)}</td>
      </tr>
    `;
  }).join('');
  ...
  const paymentMethodsHtml = paymentMethods.map((p: any) => {
    ...
    return `
      <tr>
        <td>${displayName}</td>
        <td class="r bold">${Number(pmAmount).toFixed(2)} ${currency}</td>
      </tr>
    `;
  }).join('');
  ...
  // Render QR Code Section if custom_qr_code exists or we have generated qrDataUrl (and is_zatca_enabled is true)
  const finalQr = qrDataUrl || invoice.custom_qr_code;
  const qrCodeHtml = (finalQr && posDetails?.is_zatca_enabled)
    ? `
  ...
  return `
    ...
    <div style="font-family: 'Outfit', 'Tajawal', Arial, sans-serif; max-width: 100%; margin: 0 auto; padding: 8px; color: #111; position: relative;">
      ...
      <table class="totals-table">
        ...
        <tr>
          <td>${t.subtotal}</td>
          <td class="r bold">${Number(subtotal).toFixed(2)} ${currency}</td>
        </tr>
        ` : ''}
        ${!posDetails?.custom_hide_tax_in_cart && Number(taxAmount) > 0 ? `
        <tr>
          <td>${t.tax}</td>
          <td class="r bold">${Number(taxAmount).toFixed(2)} ${currency}</td>
        </tr>
        ` : ''}
        ${Number(discountAmount) > 0 ? `
        <tr>
          <td>${t.discount}</td>
          <td class="r bold" style="color: red;">-${Number(discountAmount).toFixed(2)} ${currency}</td>
        </tr>
        ` : ''}
        <tr class="grand">
          <td style="border-top: 1.5px solid #111; padding-top: 6px;">${t.grandTotal} ${currency}</td>
          <td class="r bold" style="border-top: 1.5px solid #111; padding-top: 6px; font-size: 15px;">${Number(totalAmount).toFixed(2)}</td>
        </tr>
      </table>
      ...
      <div class="center" style="margin: 12px 0;">
        ${barcodeSvgHtml || `<div style="border: 1px dashed #ccc; padding: 10px; font-family: monospace; display: inline-block;">[Barcode: ${id}]</div>`}
...
export default function PrintPreview({ invoice, language = "en" }: PrintPreviewProps) {
  ...
  const processedInvoice = React.useMemo(() => {
    if (!invoice) return invoice;
    const processed = JSON.parse(JSON.stringify(invoice));
    if (posDetails?.custom_prices_include_vat && processed.items) {
      processed.items = processed.items.map((item: any) => {
        ...
        const rate = (taxRate > 0 && item.rate) ? (rawRate * (1 + taxRate)) : (item.price ?? rawRate);
        const amount = (item.qty ?? item.quantity ?? 1) * rate;
        return {
          ...item,
          rate,
          price: rate,
  ...
  const applyPrintCleanups = React.useCallback((rawHtml: string) => {
    let resultHtml = rawHtml;
    if (posDetails?.custom_hide_tax_in_cart) {
      try {
        const parser = new DOMParser();
        const doc = parser.parseFromString(resultHtml, 'text/html');
        
        // 1. Process all table rows <tr>
        doc.querySelectorAll('tr').forEach(row => {
          const text = (row.textContent || "").toLowerCase().trim();
          const hasSubtotal = text.includes('subtotal') || text.includes('net total') || text.includes('المجموع قبل') || text.includes('الإجمالي قبل') || text.includes('الصافي') || text.includes('الاجمالي قبل') || text.includes('الإجمالي قبل الضريبة');
          const hasTax = text.includes('tax') || text.includes('vat') || text.includes('الضريبة') || text.includes('ضريبة');
          const hasGrandTotal = (text.includes('grand') || text.includes('total') || text.includes('الاجمالي') || text.includes('الإجمالي')) && !hasSubtotal;
          
          if ((hasSubtotal || hasTax) && !hasGrandTotal) {
            row.remove();
          }
        });

        // 2. Process divs, paragraphs, spans, section headers, etc.
        const candidates = Array.from(doc.querySelectorAll('div, p, span, h1, h2, h3, h4, h5, h6, th, td'));
        candidates.forEach(el => {
          ...
          const isTaxLabel = 
            ...
            text === 'قيمة الضريبة' ||
            text.startsWith('tax (') ||
            text.startsWith('ضريبة (');
            
          if (isTaxLabel) {
            const parent = el.closest('tr, .row, .col, .flex') || el.parentElement || el;
            if (parent) {
              parent.remove();
            }
          }
        ...
        console.error("[posPreview] Failed to apply tax cleanup to HTML:", e);
      }
    }
    return resultHtml;
  }, [posDetails]);
  ...
  const isOfflineInvoice = 
    (typeof processedInvoice.name === 'string' && processedInvoice.name.startsWith('OFFLINE-')) ||
    (typeof processedInvoice.id === 'string' && processedInvoice.id.startsWith('OFFLINE-')) ||
    processedInvoice.synced === false ||
    processedInvoice.synced === 0 ||
    processedInvoice.status === 'Pending' ||
    !processedInvoice.name ||
  ...
  useEffect(() => {
    if (isOfflineInvoice) {
      const renderOffline = async () => {
        ...
        const company = posDetails?.company || "Sultan POS";
        const taxId = posDetails?.tax_id || "";
        const date = processedInvoice.posting_date || processedInvoice.date || new Date().toISOString().split('T')[0];
        const time = processedInvoice.posting_time || processedInvoice.time || new Date().toLocaleTimeString();
        const timestamp = `${date}T${time}`;
        
        // Use mapped totals for QR Code content so it is accurate and doesn't read 0.00
        const finalGrandTotal = processedInvoice.totalAmount || processedInvoice.grand_total || processedInvoice.base_grand_total || 0;
        const finalVatAmount = processedInvoice.taxAmount || processedInvoice.total_taxes_and_charges || 0;
        const grandTotal = finalGrandTotal.toString();
        const vatAmount = finalVatAmount.toString();
        
        const rawId = processedInvoice.name || processedInvoice.id || "Offline Order";
        const id = rawId.startsWith("OFFLINE-") ? rawId.replace("OFFLINE-", "") : rawId;

        // Generate ZATCA base64 TLV and QR code image URL if ZATCA is enabled
        if (posDetails?.is_zatca_enabled) {
          try {
        ...
        // Generate Code 128 barcode as SVG HTML
        let barcodeSvgHtmlLocal = "";
        try {
          ...
          svg.setAttribute("style", "width: 50mm; height: 12mm; display: block; margin: 0 auto;");
          barcodeSvgHtmlLocal = svg.outerHTML;
        } catch (e) {
          console.error("Failed to generate SVG barcode offline:", e);
          barcodeSvgHtmlLocal = `<div style="border: 1px dashed #ccc; padding: 10px; font-family: monospace; display: inline-block;">[Barcode: ${id}]</div>`;
        }

        // Try to fetch the cached template for this print format first
        try {
          const cachedTemplate = await dbGet<any>(APP_CACHE_STORE, `last_print_format_${printFormat}`);
          if (cachedTemplate && cachedTemplate.html) {
            console.log(`[Offline Print] Compiling cached template for: ${printFormat}`);
            try {
              let compiledHtml = compileOfflinePrintFormat(cachedTemplate.html, cachedTemplate.invoiceData, processedInvoice);
              
              if (barcodeSvgHtmlLocal) {
                const barcodeRegex = /<img[^>]*(?:orcascan|barcode|get_barcode|get_barcode_image|custom_barcode)[^>]*>/gi;
                compiledHtml = compiledHtml.replace(barcodeRegex, barcodeSvgHtmlLocal);
              }
              
              if (posDetails?.is_zatca_enabled && qrDataUrl) {
                compiledHtml = compiledHtml.replace(/src=["']([^"']*(?:get_zatca_qr|get_invoice_qr|get_qr_code|custom_invoice_qr_code)[^"']*)["']/gi, `src="${qrDataUrl}"`);
              }

              setHtml(applyPrintCleanups(compiledHtml));
              setStyle(cachedTemplate.style || "");
              setLoading(false);
              return;
            } catch (compileErr) {
              console.error("[Offline Print] Failed to compile cached template:", compileErr);
            }
          }
        } catch (e) {
          console.error("Failed to load cached print format offline:", e);
        }

        const offlineHtml = generateOfflineReceiptHTML(processedInvoice, posDetails, language, qrDataUrl, barcodeSvgHtmlLocal);
        setHtml(applyPrintCleanups(offlineHtml));
        setStyle("");
        setLoading(false);
      };
    ...
    const fetchPrintHTML = async () => {
      ...
      setLoading(true);
      try {
        const invoiceName = typeof processedInvoice.name === 'string' ? processedInvoice.name : '';
        const isPosInvoice = invoiceName.includes("PSINV") || processedInvoice.doctype === "POS Invoice";
        const invoiceForAPI: { doctype: string; name: string; [key: string]: unknown } = {
          ...processedInvoice,
          doctype: isPosInvoice ? 'POS Invoice' : 'Sales Invoice',
          name: invoiceName
        };
        const printResult = await getPrintFormatHTML(invoiceForAPI, printFormat);
        
        // If printResult is completely empty or returns undefined template text
        if (!printResult || !printResult.html || printResult.html.includes("undefined")) {
          console.warn("[Print Preview] Returned HTML is invalid/undefined. Falling back to offline generator.");
          let qrDataUrl = "";
          let barcodeDataUrl = "";
          const company = posDetails?.company || "Sultan POS";
          const taxId = posDetails?.tax_id || "";
          ...
          if (posDetails?.is_zatca_enabled) {
            try {
              ...
              const tag2 = getTlvString(2, taxId);
              const tag3 = getTlvString(3, timestamp);
              const tag4 = getTlvString(4, finalGrandTotal.toString());
              const tag5 = getTlvString(5, finalVatAmount.toString());
              const totalLength = tag1.length + tag2.length + tag3.length + tag4.length + tag5.length;
              const combined = new Uint8Array(totalLength);
              let offset = 0;
              [tag1, tag2, tag3, tag4, tag5].forEach(tag => {
              ...
              console.error("ZATCA QR error during fallback:", e);
            }
          }
          
          try {
            const canvas = document.createElement("canvas");
            JsBarcode(canvas, invoiceName || "Order", { format: "CODE128", width: 2, height: 48, displayValue: false });
            barcodeDataUrl = canvas.toDataURL("image/png");
          } catch (e) {}

          const offlineHtml = generateOfflineReceiptHTML(processedInvoice, posDetails, language, qrDataUrl, barcodeDataUrl);
          setHtml(applyPrintCleanups(offlineHtml));
          setStyle("");
        } else {
          let processedHtml = printResult.html;
          
          try {
            const canvas = document.createElement("canvas");
            JsBarcode(canvas, invoiceName || "Order", {
              format: "CODE128",
              width: 2,
              height: 48,
              displayValue: false
            });
            const localBarcodeDataUrl = canvas.toDataURL("image/png");
            processedHtml = processedHtml.replace(/src=["']([^"']*(?:orcascan|barcode|get_barcode|get_barcode_image|custom_barcode)[^"']*)["']/gi, `src="${localBarcodeDataUrl}"`);
          } catch (e) {
            console.error("Failed to replace barcode locally:", e);
          }
          ...
          if (posDetails?.is_zatca_enabled) {
            try {
              ...
              const binString = Array.from(combined, (byte) => String.fromCharCode(byte)).join("");
              const base64Tlv = btoa(binString);
              const generatedQr = await QRCode.toDataURL(base64Tlv, { margin: 1, width: 120 });
              
              processedHtml = processedHtml.replace(/src=["']([^"']*(?:get_zatca_qr|get_invoice_qr|get_qr_code|custom_invoice_qr_code)[^"']*)["']/gi, `src="${generatedQr}"`);
            } catch (e) {
              console.error("ZATCA QR error during HTML replacement:", e);
            }
          }
          setHtml(applyPrintCleanups(processedHtml));
          setStyle(printResult.style || "");
        }
      } catch (err) {
    ...
    fetchPrintHTML();
  }, [processedInvoice, posDetails, posLoading, printFormat, isOfflineInvoice, language, applyPrintCleanups]);

  if (loading) return <p>Loading Print Preview...</p>;

  const safeStyle = style || "";
  const styleBlock = safeStyle.trim().toLowerCase().includes("<style")
    ? safeStyle
